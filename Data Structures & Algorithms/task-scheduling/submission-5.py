...
class Solution:
    def leastInterval(self, tasks: List[str], n: int) -> int:
        task_max_heap = [-freq for _, freq in Counter(tasks).items()]
        ...
        while task_max_heap or pending_tasks:
            ...
            if not task_max_heap:
                ...
                if -heap_first > 1:
                    # Additive inverses are stored, so heap_first + 1 moves the count closer to 0.
                    task_reset_time = time + n + 1
                    pending_tasks.append((heap_first + 1, task_reset_time))
