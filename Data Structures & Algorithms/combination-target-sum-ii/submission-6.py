...
class Solution:
    def combinationSum2(self, candidates: List[int], target: int) -> List[List[int]]:
        ...
        def backtrack(cur_sum: int, ele_idx: int, cur_path: List[int]) -> None:
            ...
            if ele_idx < len(elements) and elements[ele_idx][1] > 0 and cur_sum + elements[ele_idx][0] <= target:
            ...
            if ele_idx + 1 < len(elements) and cur_sum + elements[ele_idx + 1][0] <= target:
